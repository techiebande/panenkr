...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-6 rounded-[14px] border border-border/60 py-6 shadow-sm transition-all duration-200 hover:shadow-md hover:translate-y-[-1px] hover:border-border",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-2xl leading-tight font-semibold tracking-tight", className)}
      {...props}
    />
  )
}
